...
bool MotionPlanner::isPathSafe(const robot_path_t& path) const
{
    ...
    for(auto pose: path.path){
        ...
        Point<double> current_coords(pose.x, pose.y);
        Point<int> current_cell = global_position_to_grid_cell(current_coords, distances_);
        if (!distances_.isCellInGrid(current_cell.x, current_cell.y) )
            return false;
        else if(distances_(current_cell.x,current_cell.y) < searchParams_.minDistanceToObstacle) 
            return false;
