...
#include <planning/motion_planner.hpp> // -> astar.hpp ->astar.cpp -> namespace std
...
bool test_saved_poses(const std::string& mapFile, const std::string& posesFile, const std::string& testName)
{
    ...
    for(int n = 0; n < numGoals; ++n)
    {
        ...
        if(!path.path.empty())
        {
            ...
            printf("foundPath! %d %d %d %d\n",goalCell.x,goalCell.y,endCell.x,endCell.y);
        }
