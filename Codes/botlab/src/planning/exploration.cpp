...
int8_t Exploration::executeExploringMap(bool initialize)
{
    ...
    *       -- You will likely be able to see the frontier before actually reaching the end of the path leading to it.
    ...
    // cout<<"Number of Frontiers: "<<frontiers_.size()<<endl;
    // robot_path_t path = plan_path_to_frontier(frontiers_, currentPose_, currentMap_, planner_);
    // printf("Sending A* Path, path length is %zu\n",path.path.size());
    // lcmInstance_->publish(CONTROLLER_PATH_CHANNEL, &path);
    // printf("Catching New Slam Pose! ");
    // lcmInstance_->subscribe(SLAM_POSE_CHANNEL, &Exploration::handlePose, this);

    // copyDataForUpdate();
    // float dist = distance_between_points(Point<float>(currentPose_.x, currentPose_.y),
    //                                      Point<float>(currentTarget_.x, currentTarget_.y));

    // currentPath_ = path;
    // currentTarget_ = path.path.back();
    ...
    { 
        ...
        printf("continue: ");
        double distance = distance_between_points(Point<float>(currentTarget_.x, currentTarget_.y), 
    ...
    else if(currentPath_.path.size() > 0)
    {
        status.status = exploration_status_t::STATUS_IN_PROGRESS;
    }
...
int8_t Exploration::executeReturningHome(bool initialize)
{
    ...
    */
    ...
    // robot_path_t path = planner_.planPath(currentPose_, homePose_);
    // lcmInstance_->publish(CONTROLLER_PATH_CHANNEL, &path);
    // // float dist = distance_between_points(Point<float>(currentPose_.x, currentPose_.y),
    // //                                      Point<float>(currentTarget_.x, currentTarget_.y));

    // // if reached home pose :
    // currentPath_ = path;
    ...
    std::cout << kReachedPositionThreshold << std::endl;
